...
class Db:
    ...
    def GetConfigValue(self, name, default=None):
      # Return a global (ie not specific to this client) config value
        result = self.cn.cursor().execute(
            "select value from config where key=?", (name,)).fetchone()
        if result == None:
            return default
        else:
            return result[0]
    ...
    def GetPrefs(self):
      # Return a dictionary containing all the user preference values stored for this application
        c = self.cn.cursor()
        c.execute("select key,value from config where key like '" +
                  self.prefsPrefix + "%'")
    ...
    def GetData(self, t, now):
      ...
      # All data appearing on or after time 't' will be returned
        c = self.cn.cursor()
        c.execute('select ts,dl,ul from data where ts >=' +
                  str(t) + ' order by ts')
