...
class OptionsDialog(wx.Dialog):
    def __init__(self, parent, prefs, capabilities, PrefsUpdated):
      ...
      # The 'Scale' value fields
        ...
        self.scaleTxt = wx.TextCtrl(
            self, -1, str(prefs.Get('scale')), size=(100, -1))
      ...
      # The Opacity slider
        if self.capabilities['opacity']:
            ...
            self.opacitySlider = wx.Slider(self, size=(
                150, -1), style=wx.SL_HORIZONTAL | wx.SL_AUTOTICKS | wx.SL_LABELS)
      ...
      # Do all the layout...
        ...
        if self.capabilities['opacity']:
            gridSizer.AddMany(
                [(lPad, 0), opacityLabel, self.opacitySlider, (rPad, 0)])
        gridSizer.AddMany([(lPad, 0), floatLabel, self.floatChk, (rPad, 0)])
        if self.capabilities['clickthru']:
            gridSizer.AddMany([(lPad, 0), clickThruLabel,
                               self.clickThruChk, (rPad, 0)])
    ...
    def PromptForColour(self, name):
      # Display the colour picker dialog
        ...
        if dlg.ShowModal() == wx.ID_OK:
          # User picked a colour and clicked 'OK' so add the new colour to the prefs dictionary
